...
El cambio que hemos hecho sobre el prototipo es pasar movimientos por referencia para inicializarlo en el main a cero, 
la primera vez que se usa el programa. Si lo inicializasemos aquí se pondría a 0 cada vez que se ejecutase una opción. */
...
void ejecutarOpc(int opc, tArray fila, int &tam, int &movimientos){ 
	bool movimientohecho, filaordenada;
		...
		case 1: {
			...
			movimientos = 0;
			if (tam != 0){
				mostrarfila(fila, tam); // Sólo se muestra la fila cuando esta no está vacía.
			}
		}
				break;
		case 2: {
		...
		case 3: {
			if (tam != 0){
				...
				cout << endl;
				if (movimientos == 1){
					cout << "Usted lleva hecho " << movimientos << " movimiento" << endl;
				}
				else { cout << "Usted lleva hechos " << movimientos << " movimientos" << endl; }
	...
	if (archivo.is_open()){
		leerFilaFich(archivo, fila, tam);
			...
			if (tam != 0){
				...
				cout << "Se ha podido abrir el fichero pero éste está vacío." << endl;
			}
	}
	else { 
		cout << endl;
		cout << "El archivo no se pudo abrir" << endl; 
		/* Distinguimos entre el caso de que la fila se haya reinicializado como vacía 
		al no poderse abrir un archivoestando ya cargada una fila con tamaño no nulo o que
		la fila que le precede si tuviese tamaño 
		(por defecto antes de cargar una fila el tamaño es 0). */
		if (tam != 0){
			cout << "La fila ha sido reinicializada como vacía." << endl;
		}
		else { cout << "La fila sigue vacía." << endl; }
		tam = 0;
	}
...
void ejecutarGuardarfichero(const tArray fila, int tam){
	...
	bool acabaentxt;
	ofstream archivo2;
	/* Hemos decidido controlar que el usuario te introduzca un archivo acabado en '.txt',
	para que pueda volver a utilizarlo en el programa. En caso contrario el programa sigue pidiendo un nombre correcto. */
	do{
	...
	if (archivo2.is_open()){
		...
		}                          // En caso de que la fila guarada sea vacía informa al usuario y no la muestra (en EjecutarOPc)
		else {
			cout << "La fila guardada está vacía." << endl;
		}
	}
	else {
		cout << endl;
		cout << "No se ha podido abrir el archivo y por tanto la fila no se guardó." << endl; }
...
void escribirFilaFich(ostream &fich, const tArray fila, int tam){
	...
	fich << CENTINELA;  // Se introduce un centinela al final del archivo para que este pueda ser usado por el programa como archivo de entrada
}
...
void mostrarfila(const tArray fila, int tam){
	...
	for (i = 0; i < tam; i++){
		cout << "|";
		if (fila[i] == PROD_NULO){ cout << setw(2) << " "; }  // Con este bucle mostramos los materiales dejando un hueco vacío cuando sea 0
		else { cout << setw(2) << fila[i]; }
	}
	cout << "|" << endl;

	for (i = 0; i < (3 * tam + 1); i++){    // Con este bucle hacemos la separación en guiones
		cout << "-";
	}
	cout << endl;
...
bool sonPosicionesPosiblesGrua(int tam, int posIni, int posFin, int posSoltar){
	...
	posfinsoltar = posSoltar + distanciasegmento;        // Evaluamos que los 4 extremos implicados en el movimiento de la grúa sean válidos
	inivalida = esPosValida(tam, posIni);
...
bool esPosibleGrua(const tArray fila, int tam, int posIni, int posFin, int posSoltar){
	int distanciasegmento, i, posempezar, suma;
	bool gruarealizable, pertenecealintervalo, posvalidas, yanosolapa;
	distanciasegmento = posFin - posIni;
	gruarealizable = true;
	yanosolapa = false;
	i = 0;
	if (posIni <= posSoltar){ 
		posempezar = posSoltar; 
		suma = 1;
	}
	else { 
		posempezar = posSoltar + distanciasegmento;
		suma = -1;
	}
	posvalidas = sonPosicionesPosiblesGrua(tam, posIni, posFin, posSoltar);
	if (posvalidas){   //Primero comprobamos que los 4 extremos sean correctos
		while (i <= distanciasegmento && gruarealizable && !yanosolapa){
			pertenecealintervalo = perteneceABloqueLevantado((posempezar + i), posIni, posFin);
			if (!pertenecealintervalo){ yanosolapa = true; }
			/* Recibimos un booleano que nos indica si la posicion que se está evaluando
			pertenece al intervalo de las que estamos levantando, pues ya si la levantamos, ya no estará ahí */
			if (!estaVacia(fila, posIni + i) && !pertenecealintervalo && !estaVacia(fila, posempezar + i)){

				// No pertenece al intervalo para tener en cuenta que las que he levantado ahora son vacias
				// Tambien tengo en cuenta si lo que va a caer sobre donde hay matrial es posicion en blanco
				// Y también si lo que estamos soltando es una posición vacía (no aplastará nada)
				gruarealizable = false;
			}
			i = i + suma;;
		}
	}
	else { gruarealizable = false; }  // Pero si las posiciones no son correctas, el movimiento es imposible
...
int posHuecoLibreDir(const tArray fila, int tam, int posIni, int direccion){
	...
	if (!encontrado){ primerhuecolibre = tam; } // Devolvemos 'tam' siempre que no haya ningún hueco libre en la dirección introducida
...
void ejecutarExcavadora(tArray fila, int tam, bool &movimientohecho){
	...
	while (direccion != 1 && direccion != -1){       // Meto un bucle dentro de otro para que si mete un numero no valido y luego algo que no sea un entero, no se vuelva loco
	...
	/* Llamamos a todas estas funciones para diferenciar entre hacer solo algún movimiento, hacerlos todos,
	...
	if (!esposvalida){
		...
		cout << endl;
		cout << "No se han podido hacer todos los desplazamientos, pero sí alguno." << endl << "Revise el número de movimientos introducidos. No se pueden empujar materiales contra la pared " << endl;
		cout << "La fila ha quedado del siguiente modo: " << endl;
	}
	else { 
		cout << endl;
